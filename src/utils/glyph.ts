...
type ThemeProp = 'class' | 'wrapperClass' | 'contentClass' | 'style' | 'contentStyle' | 'color' | 'fillMode';
...
function normalizeConfig(color: string, config: any, defaultProfile: any = _defaultProfile): Profile<Glyph> {
	let rootColor = color;
	let root: any = {};
	...
	const result = defaultsDeep(root, { start: root.startEnd, end: root.startEnd }, defaultProfile) as Profile<any>;
...
export class HighlightRenderer implements GlyphRenderer<Highlight> {
	...
	render({ data, onStart, onEnd }: DateRangeCell<Attribute>, glyphs: Record<string, Highlight[]>) {
...
export class BaseRenderer<T extends Partial<Glyph>> implements GlyphRenderer<T> {
	type = '';
	collectionType = '';
	...
	render({ data, onStart, onEnd }: DateRangeCell<Attribute>, glyphs: Record<string, T[]>) {
		const { key } = data;
