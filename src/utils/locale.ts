import { type PageConfig, type CachedPage, getPageKey, getCachedPage, getPage } from './page';
import {
	type DateSource,
	type DateOptions,
	type DayOfWeek,
...
import { isString, isNumber, isDate, isObject, has, pick, clamp, defaultsDeep } from './helpers';
...
export function resolveConfig(config: string | Partial<LocaleConfig> | undefined, locales: any) {
	...
	const result: LocaleConfig = isObject(config) ? defaultsDeep(config, defLocale) : defLocale;
...
export default class Locale {
	...
	constructor(config: Partial<LocaleConfig> | string | undefined = undefined, timezone?: string) {
		const { id, firstDayOfWeek, masks, monthCacheSize, pageCacheSize } = resolveConfig(
			config,
	...
	toDate(d: DateSource | Partial<SimpleDateParts>, opts: Partial<DateOptions> = {}): Date {
		...
		if (result && (patch || rules)) {
			...
			if (patch && fillDate != null) {
				...
				parts = this.getDateParts(this.toDate({ ...fillParts, ...pick(parts, DatePatchKeys[patch]) }));
			}
	...
	getThisMonthParts() {
		const date = new Date();
		return this.getMonthParts(<MonthInYear>(date.getMonth() + 1), date.getFullYear());
	}
