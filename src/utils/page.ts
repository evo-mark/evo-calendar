...
function getDays(
	{ monthComps, prevMonthComps, nextMonthComps }: Pick<Page, 'monthComps' | 'prevMonthComps' | 'nextMonthComps'>,
	locale: Locale,
): CalendarDay[] {
	const days: CalendarDay[] = [];
	const { firstDayOfWeek, firstWeekday, isoWeeknumbers, weeknumbers, numDays, numWeeks } = monthComps;
...
export function getPageAddressForDate(date: DateSource, view: PageView, locale: Locale) {
	return pick(locale.getDateParts(locale.toDate(date)), viewAddressKeys[view]) as PageAddress;
}
...
export function pageIsBeforePage(page: PageAddress | null | undefined, comparePage: PageAddress | null | undefined) {
...
export function pageIsAfterPage(page: PageAddress | null | undefined, comparePage: PageAddress | null | undefined) {
	if (!pageIsValid(page) || !pageIsValid(comparePage)) return false;
	page = page as PageAddress;
	comparePage = comparePage as PageAddress;
...
export function pageIsEqualToPage(aPage: PageAddress | null | undefined, bPage: PageAddress | null | undefined) {
	...
	return (
		aPage.year === bPage.year && aPage.month === bPage.month && aPage.week === bPage.week && aPage.day === bPage.day
	);
}

export function pageRangeToArray(from: PageAddress, to: PageAddress, view: PageView, locale: Locale) {
...
export function getPage(config: PageConfig, cachedPage: CachedPage) {
	...
	switch (view) {
		...
		default: {
			...
			page.viewWeeks = page.weeks.slice(0, trimWeeks ? page.monthComps.numWeeks : undefined);
