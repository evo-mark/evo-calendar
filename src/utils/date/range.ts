...
export class DateRange {
	...
	startsOnDay(dayParts: DayParts) {
		return this.start?.dayIndex === dayParts.dayIndex || !!this.repeat?.passes(dayParts);
	}
	...
	intersectsRange(range: DateRange) {
		return this.intersectsDayRange(range.start?.dayIndex ?? -Infinity, range.end?.dayIndex ?? Infinity);
	}
