...
export class DateRepeat implements Rule<GroupRuleType> {
	...
	constructor(
		...
		if (isFunction(config)) {
			...
			this.from = config.from ? this.locale.getDateParts(config.from) : parent?.from;
			this.until = config.until ? this.locale.getDateParts(config.until) : parent?.until;
	...
	getObjectRules(config: any) {
		...
		if (config.on != null) {
			...
			rules.push(new DateRepeat(config.on, { locale: this.locale }, this.parent));
		}
