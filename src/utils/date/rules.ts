...
export type OrdinalArrayConfig = SingleOrArray<[OrdinalWeekInMonth, ...DayOfWeek[]]>;
...
export class DaysRule extends ComponentRule {
	constructor(components: unknown) {
		super(ComponentRuleType.Days, components, isDayInMonth, ({ day, dayFromEnd }) => [day, -dayFromEnd]);
	}
}

export class WeekdaysRule extends ComponentRule {
	constructor(components: unknown) {
		super(ComponentRuleType.Weekdays, components, isDayOfWeek, ({ weekday }) => [weekday]);
	}
}

export class WeeksRule extends ComponentRule {
	constructor(components: unknown) {
		super(ComponentRuleType.Weeks, components, isWeekInMonth, ({ week, weekFromEnd }) => [week, -weekFromEnd]);
	}
}

export class MonthsRule extends ComponentRule {
	constructor(components: unknown) {
		super(ComponentRuleType.Months, components, isMonthInYear, ({ month }) => [month]);
	}
}
	...
	passes(dayParts: DayParts) {
		...
		return this.components.some(
			([ordinalWeek, ordinalWeekday]) =>
				(ordinalWeek === weekdayOrdinal || ordinalWeek === -weekdayOrdinalFromEnd) &&
