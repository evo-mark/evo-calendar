...
export function getPopoverEventHandlers(opts: Partial<PopoverOptions>): Partial<PopoverEventHandlers> {
	...
	const focusOutHandler = (e: FocusEvent) => {
		if (focused && !elementContains(e.currentTarget as Node, e.relatedTarget as Element)) {
			focused = false;
...
const addHandlers = (target: Element | ComponentPublicInstance | string, opts: Partial<PopoverOptions>) => {
	const el = resolveEl(target);
	if (el == null) return;
