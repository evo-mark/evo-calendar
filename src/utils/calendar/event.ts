...
export function createEvent(config: Partial<EventConfig>, ctx: EventContext) {
	...
	const minDurationMs = computed(() => state.minDurationMinutes * MS_PER_MINUTE);
	const maxDurationMs = computed(() => state.maxDurationMinutes * MS_PER_MINUTE);
	...
	const durationMs = computed(() => endDate.value.getTime() - startDate.value.getTime());
	...
	const dragIsDirty = computed(() => {
		...
		return dragOrigin.start.getTime() !== startDateTime.value || dragOrigin.end.getTime() !== endDateTime.value;
	});
	...
	function startDrag(day: CalendarDay) {
		...
		const maxOffsetWeeks = ctx.isMonthly.value ? ctx.dayRows.value - day.weekPosition : 0;
		const minOffsetWeekdays = ctx.isDaily.value ? 0 : -day.weekdayPosition + 1;
		const maxOffsetWeekdays = ctx.isDaily.value ? 0 : ctx.dayColumns.value - day.weekdayPosition;
	...
	function updateDrag(offset: DragOffset) {
		...
		const weekdaysToAdd = clamp(offset.weekdays, minOffsetWeekdays, maxOffsetWeekdays);
		const daysToAdd = weeksToAdd * ctx.dayColumns.value + weekdaysToAdd;
