...
import { type DateSource, addDays, addMonths, addYears } from '../utils/date/helpers';
import { type DateRangeCell, DateRangeContext } from '../utils/date/range';
import { getDefault } from '../utils/defaults';
import { type CustomElement, arrayHasItems, has, head, isBoolean, last } from '../utils/helpers';
import {
	type CalendarDay,
	type CalendarWeek,
	type Page,
...
export type DayCells = Record<number, { day: CalendarDay; cells: DateRangeCell<Attribute>[] }>;
...
export function createCalendar(props: CalendarProps, { slots, emit }: Pick<SetupContext, 'slots' | 'emit'>) {
	// #region Refs
	...
	const { theme, color, displayMode, locale, masks, minDate, maxDate, disabledAttribute, disabledDates } =
	...
	const minPage = computed(() => props.minPage || (minDate.value ? getDateAddress(minDate.value) : null));

	const maxPage = computed(() => props.maxPage || (maxDate.value ? getDateAddress(maxDate.value) : null));
	...
	const addPages = (address: PageAddress, count: number, view = _view.value) => {
	...
	const getPageForAttributes = () => {
		...
		const attr = attributes.value.find((attr) => attr.pinPage) || attributes.value[0];
	...
	const getTargetPageRange = (page: PageAddress, opts: Partial<MoveOptions> = {}) => {
	...
	const getPageTransition = (oldPage: Page, newPage: Page, defaultTransition = '') => {
	...
	const canMove = (target: MoveTarget, opts: Partial<MoveOptions> = {}) => {
		const page = pageIsValid(target as PageAddress) ? (target as Page) : getDateAddress(target as DateSource);
		...
		const pagesInRange = pageRangeToArray(opts.fromPage!, opts.toPage!, _view.value, locale.value).map((p) =>
			pageIsBetweenPages(p, minPage.value, maxPage.value),
		);
