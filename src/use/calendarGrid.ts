import { type ExtractPropTypes, type PropType, computed, inject, nextTick, provide, ref, watch } from 'vue';
import CalendarCellPopover from '../components/CalendarGrid/CalendarCellPopover.vue';
import { type Event, type EventConfig, createEvent as _createEvent } from '../utils/calendar/event';
...
type GridState = 'NORMAL' | 'CREATE_MONITOR' | 'DRAG_MONITOR' | 'RESIZE_MONITOR';
...
class Messages {
	...
	static EventResizeUpdate(event: Event, offset: ResizeOffset) {
		return new EventResizeMessage(this._emit, 'event-resize-update', event, offset);
	}
...
class CancellableEventMessage<T extends Event | EventConfig> extends EventMessage<T> {
	cancel = false;
}
...
class EventResizeMessage extends CancellableEventMessage<Event> {
	...
	constructor(emit: Function, type: MessageType, event: Event, offset?: ResizeOffset) {
...
export function createCalendarGrid(props: CalendarGridProps, { emit, slots }: any) {
	...
	const { view, isDaily, isMonthly, pages, locale, move, onDayFocusin } = calendar;
	...
	function getDateFromPosition(position: number, day: CalendarDay, offsetMs = 0, snapMs = 0) {
	...
	const getPositionFromMouseEvent = (gridEl: IBoundingRect | null, event: MouseEvent): Point => {
	...
	const getPositionFromTouchEvent = (gridEl: IBoundingRect | null, event: TouchEvent): Point => {
		if (gridEl == null) return { x: 0, y: 0 };
		const rect = gridEl.getBoundingClientRect();
	...
	const getPositionFromUIEvent = (gridEl: IBoundingRect | null, event: UIEvent): Point => {
		if (event.type.startsWith('touch')) return getPositionFromTouchEvent(gridEl, event as TouchEvent);
	...
	function startResizingEvents(position: number, day: CalendarDay, event: Event, isStart: boolean, isNew: boolean) {
	...
	function startDraggingEvents(position: number, day: CalendarDay, event: Event) {
		if (active.value) return;
		dragging.value = true;
	...
	function handleNormalEvent(gse: GridStateEvent, day: CalendarDay, position: number, evt: Event | undefined) {
		switch (gse) {
	...
	function handleResizeMonitorEvent(event: GridStateEvent, position: number, day: CalendarDay) {
	...
	function handleDragMonitorEvent(event: GridStateEvent, day: CalendarDay, position: number) {
	...
	function updateState(gse: GridStateEvent, day: CalendarDay, position: number, evt: Event | undefined = undefined) {
		switch (state.value) {
			case 'NORMAL': {
	...
	const setActiveGrid = (event: MouseEvent | TouchEvent) => {
		activeGridRef.value =
			[dailyGridRef, weeklyGridRef].find((ref) => ref.value && ref.value.contains(event.currentTarget as Node))
				?.value ?? null;
	};
	...
	const handleEvent = (
		...
		const eventName = (event.shiftKey ? `${stateEvent}_SHIFT` : stateEvent) as GridStateEvent;
