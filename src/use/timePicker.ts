...
import { type DateParts, type SimpleDateParts, getDatePartsOptions, isDateParts } from '../utils/date/helpers';
...
export function createTimePicker(props: TimePickerProps) {
	...
	const parts = computed(() => dateParts.value[props.position] || { isValid: false });
	...
	const options = computed(() => getDatePartsOptions(parts.value as DateParts, rules.value[props.position]));
	...
	const isAMOptions = computed(() => {
		const result = [];
		if (arrayHasItems(amHourOptions.value)) result.push({ value: true, label: 'AM' });
		if (arrayHasItems(pmHourOptions.value)) result.push({ value: false, label: 'PM' });
		return result;
	});
