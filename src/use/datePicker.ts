...
export function createDatePicker(props: DatePickerProps, { emit, slots }: SetupContext<string[]>) {
	...
	const valueStart = computed(() =>
		isRange.value && dateValue.value != null ? (dateValue.value as SimpleDateRange).start : null,
	);

	const valueEnd = computed(() =>
		isRange.value && dateValue.value != null ? (dateValue.value as SimpleDateRange).end : null,
	);
	...
	const dateParts = computed(() => getDateParts(dragValue.value ?? dateValue.value));
	...
	const inputMask = computed(() => {
		...
		if (isDateTimeMode.value) {
			return props.is24hr ? masks.value.inputDateTime24hr : masks.value.inputDateTime;
		}
	...
	const rules = computed(() => {
		return normalizeConfig(props.rules === 'auto' ? getAutoRules() : (props.rules ?? {}));
	});
	...
	function normalizeDateConfig(config: Partial<DateConfig> | Partial<DateConfig>[]): DateConfig[] {
	...
	function hasDateValue(value: DatePickerDate): value is Exclude<DatePickerDate, null> {
	...
	function hasValue(value: DatePickerModel): value is Exclude<DatePickerDate, null> | DatePickerRangeObject {
		return hasRangeValue(value) || hasDateValue(value);
	}

	function valuesAreEqual(a: null | Date | SimpleDateRange, b: null | Date | SimpleDateRange): boolean {
	...
	function denormalizeValue(value: null | Date | SimpleDateRange, config: DateConfig[]) {
		if (hasRangeValue(value)) {
			return {
	...
	function updateValue(value: any, opts: Partial<UpdateOptions> = {}): Promise<ReturnType<typeof forceUpdateValue>> {
	...
	function formatInput() {
		nextTick(() => {
			...
			const value = denormalizeValue(dragValue.value ?? dateValue.value, config);
			if (isRange.value) {
				// @ts-ignore
	...
	function onInputUpdate(inputValue: string, target: ValueTarget, opts: Partial<UpdateOptions>) {
	...
	async function moveToValue(target: ValueTarget, opts: Partial<MoveOptions> = {}) {
