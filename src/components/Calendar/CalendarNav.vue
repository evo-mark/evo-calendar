<template>
	<!--Nav panel-->
	<div ref="navContainer" class="vc-nav-container">
		<!--Nav header-->
		<div class="vc-nav-header">
			...
			>
				<CalendarSlot name="nav-prev-button" :move="movePrev" :disabled="!prevItemsEnabled">
			...
			>
				<CalendarSlot name="nav-next-button" :move="moveNext" :disabled="!nextItemsEnabled">
		...
		<div class="vc-nav-items">
			<button
				...
				:class="[item.isActive ? 'is-active' : item.isCurrent ? 'is-current' : '']"
...
function focusFirstItem() {
	...
	setTimeout(() => {
		...
		const focusableEl = navContainer.value.querySelector('.vc-nav-item:not(:disabled)') as HTMLElement;
		if (focusableEl) {
			focusableEl.focus();
		}
	}, 10);
...
const monthItems = computed(() =>
	getMonthItems(selectedYear.value, masks.value.navMonths).map((item) => ({
		...item,
		click: () => move({ month: item.month, year: item.year }, { position: page.value.position }),
	})),
);

const prevMonthItems = computed(() => getMonthItems(selectedYear.value - 1, masks.value.navMonths));

const prevMonthItemsEnabled = computed(() => prevMonthItems.value.some((i) => !i.isDisabled));

const nextMonthItems = computed(() => getMonthItems(selectedYear.value + 1, masks.value.navMonths));

const nextMonthItemsEnabled = computed(() => nextMonthItems.value.some((i) => !i.isDisabled));

const yearItems = computed(() =>
	getYearItems(getStartYear(selectedYearGroup.value), getEndYear(selectedYearGroup.value)).map((item) => {
		return {
			...item,
			click: () => {
...
const prevYearItems = computed(() =>
	getYearItems(getStartYear(selectedYearGroup.value - 1), getEndYear(selectedYearGroup.value - 1)),
);

const prevYearItemsEnabled = computed(() => prevYearItems.value.some((i) => !i.isDisabled));

const nextYearItems = computed(() =>
	getYearItems(getStartYear(selectedYearGroup.value + 1), getEndYear(selectedYearGroup.value + 1)),
);

const nextYearItemsEnabled = computed(() => nextYearItems.value.some((i) => !i.isDisabled));
