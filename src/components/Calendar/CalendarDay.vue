<template>
	<div :class="dayClasses">
		...
		<CalendarSlot
			...
			:locale="locale"
		>
			<div v-popover="dayPopover" v-bind="dayContentProps" v-on="dayContentEvents">
				{{ day.label }}
			</div>
		</CalendarSlot>
		<!--Dots-->
...
export default defineComponent({
	...
	setup(props) {
		...
		const attributes = computed(() => attributeCells.value.map((cell) => cell.data as Attribute));
		...
		const popoverAttrs = computed(() => popovers.value.map((p: any) => p.attribute));
