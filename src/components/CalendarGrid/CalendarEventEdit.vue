	...
	<div class="w-80">
		...
		<div class="flex items-center justify-between py-1 px-2 space-x-3">
			...
			<div class="flex justify-end items-center space-x-3">
				...
				<!--Remove button-->
				<button type="button" class="w-5 h-5 text-red-500 hover:opacity-50" @click="onRemove">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
				...
				<!--Close button-->
				<button type="button" class="w-5 h-5 hover:opacity-50" @click="$emit('close')">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
		...
		<div v-else class="py-2 px-3 space-y-2">
			...
			<div class="flex items-center">
				...
				<div class="ml-2 text-gray-200 font-medium">
					...
					<span v-else-if="event.isMultiDay">{{ event.startTimeLabel }}-{{ event.endTimeLabel }}</span>
					<span v-else>{{ event.startTimeLabel }}-{{ event.endTimeLabel }}</span>
				</div>
			</div>
		</div>
	</div>
