<template>
	...
	>
		...
		>
			<div v-if="isVisible" tabindex="-1" :class="`vc-popover-content direction-${direction}`" v-bind="$attrs">
				<slot :direction="direction" :alignment="alignment" :data="data" :hide="hide">
					{{ data }}
				</slot>
				<span :class="['vc-popover-caret', `direction-${direction}`, `align-${alignment}`]" />
			</div>
		</Transition>
	</div>
</template>
...
import type { PopoverEvent, PopoverOptions, PopoverState } from '../../utils/popovers';
...
export default defineComponent({
	...
	props: {
		...
		boundarySelector: { type: String, default: '' },
	},
	...
	setup(props, { emit }) {
		...
		const popperOptions = computed<Partial<OptionsGeneric<any>>>(() => {
			return {
				...
				strategy: (state.positionFixed ? 'fixed' : 'absolute') as PositioningStrategy,
				boundary: '',
				modifiers: [
					{
						name: 'onUpdate',
		...
		function setupPopper() {
			nextTick(() => {
				...
				if (!popper) {
					popper = createPopper(el as Element, popoverRef.value, popperOptions.value);
				} else {
					popper.update();
				}
			});
		...
		function onDocumentClick(e: CustomEvent) {
			...
			if (elementContains(popoverRef.value, target) || elementContains(popperRef as Node, target)) {
				return;
			}
			// Hide the popover
			hide({ force: true });
		...
		function onMouseOver() {
			...
			if (state.isInteractive && ['hover', 'hover-focus'].includes(state.visibility)) {
				show();
			}
		}
		...
		function onFocusIn() {
			...
			if (state.isInteractive && ['focus', 'hover-focus'].includes(state.visibility)) {
				show();
			}
		}
		...
		function onFocusOut(e: FocusEvent) {
			if (
				...
				(!e.relatedTarget || !elementContains(popoverRef.value!, e.relatedTarget as Node))
			) {
